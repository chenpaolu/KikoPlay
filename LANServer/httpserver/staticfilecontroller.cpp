    ...
    :HttpRequestHandler(parent)
{
    ...
    Logger::logger()->log(Logger::LANServer, "StaticFileController: docroot=%s, encoding=%s, maxAge=%i",qPrintable(docroot),qPrintable(encoding),maxAge);
    maxCachedFileSize=settings->value("maxCachedFileSize","65536").toInt();
    cache.setMaxCost(settings->value("cacheSize","1000000").toInt());
    cacheTimeout=settings->value("cacheTime","60000").toInt();
    long int cacheMaxCost=(long int)cache.maxCost();
    Logger::logger()->log(Logger::LANServer, "StaticFileController: cache timeout=%i, size=%li",cacheTimeout,cacheMaxCost);
}
...
void StaticFileController::service(HttpRequest &request, HttpResponse &response)
{
    ...
    if (entry && (cacheTimeout==0 || entry->created>now-cacheTimeout))
    {
        ...
        mutex.unlock();
        Logger::logger()->log(Logger::LANServer, "StaticFileController: Cache hit for %s",path.data());
    ...
    else
    {
        ...
        if (file.open(QIODevice::ReadOnly))
        {
            ...
            if (file.exists())
            {
                Logger::logger()->log(Logger::LANServer, "StaticFileController: Cannot open existing file %s for reading",qPrintable(file.fileName()));
                response.setStatus(403,"forbidden");
                response.write("403 forbidden",true);
            }
            else
...
void StaticFileController::setContentType(const QString fileName, HttpResponse &response) const
{
    ...
    {
        Logger::logger()->log(Logger::LANServer, "StaticFileController: unknown MIME type for filename '%s'", qPrintable(fileName));
    }
}
